...
7. `scripts/sync_matches.py` ยัง map `matchId` จาก provider fixture id โดยตรง ซึ่งอาจไม่ตรงกับ `Match_ID` ใน Google Sheet ต้องทำ mapping ให้ชัดเจนก่อนใช้ production
8. ควรทดสอบ CORS/POST กับ Apps Script หลัง deploy จริง เพราะ Apps Script Web App มีพฤติกรรม redirect และ content-type เฉพาะตัว
9. Admin Panel แสดง PIN ในตารางผู้ใช้ ซึ่งเหมาะกับระบบทดลอง แต่ก่อน production ควรพิจารณาไม่แสดง PIN หรือเปลี่ยนวิธีเก็บ PIN ให้ปลอดภัยขึ้น
