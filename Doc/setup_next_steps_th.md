...
8. ทุกครั้งที่แก้ `backend/api.js` ต้องคัดลอกโค้ดไปวางใน Apps Script แล้วกด Deploy/Update deployment ใหม่ ไม่เช่นนั้น GitHub Actions จะยังเรียก backend เวอร์ชันเก่า
9. ควรทดสอบ CORS/POST กับ Apps Script หลัง deploy จริง เพราะ Apps Script Web App มีพฤติกรรม redirect และ content-type เฉพาะตัว
10. Admin Panel แสดง PIN ในตารางผู้ใช้ ซึ่งเหมาะกับระบบทดลอง แต่ก่อน production ควรพิจารณาไม่แสดง PIN หรือเปลี่ยนวิธีเก็บ PIN ให้ปลอดภัยขึ้น
