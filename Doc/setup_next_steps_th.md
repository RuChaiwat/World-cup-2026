...
หาก workflow แสดงว่า sync สำเร็จแต่ `Synced 0 matches` ให้ดู log ส่วน `Unmatched matches returned by Apps Script` ซึ่งจะแสดงชื่อทีมจาก provider สูงสุด 10 คู่แรก แล้วนำชื่อ `homeTeam` และ `awayTeam` ไปเทียบกับค่าใน Google Sheet tab `Matches` คอลัมน์ `Home_Team` และ `Away_Team` แบบตัวต่อตัว
...
4. หลังแก้ `backend/api.js` ต้อง redeploy Apps Script ก่อน จึงจะเห็น log `Unmatched` และ `Skipped overridden` จาก backend เวอร์ชันใหม่
