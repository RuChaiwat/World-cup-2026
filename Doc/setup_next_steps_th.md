...
10. สำหรับ GitHub Pages ให้ตั้งค่า Repository Settings → Pages → Build and deployment → Source = `GitHub Actions` แล้วใช้ workflow `.github/workflows/deploy_pages.yml` เพื่อ deploy โฟลเดอร์ `liff-app` แทน `pages-build-deployment` แบบ auto-generated ของ GitHub Pages
11. ควรทดสอบ CORS/POST กับ Apps Script หลัง deploy จริง เพราะ Apps Script Web App มีพฤติกรรม redirect และ content-type เฉพาะตัว
12. Admin Panel แสดง PIN ในตารางผู้ใช้ ซึ่งเหมาะกับระบบทดลอง แต่ก่อน production ควรพิจารณาไม่แสดง PIN หรือเปลี่ยนวิธีเก็บ PIN ให้ปลอดภัยขึ้น
...
## 10. LINE OA Rich Menu และ LIFF routing

แนะนำให้สร้าง Rich Menu ใน LINE OA จำนวน 4 ปุ่ม และใส่ URL เป็น LIFF URL พร้อม query `screen` ดังนี้:

| ปุ่ม | URL ตัวอย่าง |
| --- | --- |
| ทายผลบอล | `https://liff.line.me/<LIFF_ID>?screen=matches` |
| ทายแชมป์โลก | `https://liff.line.me/<LIFF_ID>?screen=winner` |
| ผลย้อนหลัง | `https://liff.line.me/<LIFF_ID>?screen=history` |
| ตารางคะแนน | `https://liff.line.me/<LIFF_ID>?screen=leaderboard` |

หน้า `ทายผลบอล` จะแสดงเฉพาะ match ที่เปิดให้ทายภายใน 3 วันก่อน kickoff และปิดเมื่อถึง kickoff โดย backend จะตรวจซ้ำทุกครั้งตอน submit เพื่อป้องกันการทายหลังหมดเวลา

หน้า `ผลย้อนหลัง` default เป็นวันที่ปัจจุบันตาม timezone `Asia/Bangkok` และสามารถเลือกวันที่อื่นได้จาก date picker

หน้า `ตารางคะแนน` จะแสดง Top 10 และหากผู้ใช้ที่ login อยู่ไม่ได้อยู่ใน Top 10 ระบบจะแสดงแถวของผู้ใช้นั้นเพิ่มเป็นแถวสุดท้าย


หมายเหตุ: หาก LIFF แสดง error ว่า `the permission is not in liff app scope` ให้ตรวจใน LINE Developers ว่า LIFF/LINE Login channel เปิด scope `profile` แล้ว หรือใช้เวอร์ชันล่าสุดของ frontend ซึ่ง fallback ไปใช้ `liff.getContext().userId` ได้เมื่อเปิดจาก LINE OA


### ตรวจสอบ `LINE_CHANNEL_ACCESS_TOKEN` เมื่อ Broadcast ขึ้น 401

ถ้า workflow ขึ้น `Authentication failed. Confirm that the access token in the authorization header is valid.` ให้ตรวจว่า GitHub Secret `LINE_CHANNEL_ACCESS_TOKEN` เป็น **Messaging API channel access token** จาก LINE Developers Console > Messaging API channel > Messaging API tab > Channel access token ไม่ใช่ Channel secret, LIFF ID, หรือ token ของ LINE Login channel
