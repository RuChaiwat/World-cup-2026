...
| `FOOTBALL_API_URL` | ไม่บังคับ; ค่า default คือ `https://v3.football.api-sports.io/fixtures` |
...
5. `scripts/sync_matches.py` ใช้ endpoint default `https://v3.football.api-sports.io/fixtures`; หาก network/DNS ของ runner ใช้ endpoint อื่น ให้ตั้ง secret/variable `FOOTBALL_API_URL` เพิ่มได้
6. `scripts/sync_matches.py` ยัง map `matchId` จาก Football API fixture id โดยตรง ซึ่งอาจไม่ตรงกับ `Match_ID` ใน Google Sheet ต้องทำ mapping ให้ชัดเจนก่อนใช้ production
7. ควรทดสอบ CORS/POST กับ Apps Script หลัง deploy จริง เพราะ Apps Script Web App มีพฤติกรรม redirect และ content-type เฉพาะตัว
8. Admin Panel แสดง PIN ในตารางผู้ใช้ ซึ่งเหมาะกับระบบทดลอง แต่ก่อน production ควรพิจารณาไม่แสดง PIN หรือเปลี่ยนวิธีเก็บ PIN ให้ปลอดภัยขึ้น
